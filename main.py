...
class DIVA:
    ...
    def PrepareSimulation(self):
        ...
        n_samplesperproduction = len(self.production_art[0, 0].Art)
        ...
        aud_min_tensor = torch.from_numpy(self.production_art[0, 0].Aud_min.astype(np.float))
        ...
        aud_max_tensor = torch.from_numpy(self.production_art[0, 0].Aud_max.astype(np.float))
        ...
        som_min_tensor = torch.from_numpy(self.production_art[0, 0].Som_min.astype(np.float))
        ...
        som_max_tensor = torch.from_numpy(self.production_art[0, 0].Som_max.astype(np.float))
    ...
    def ChangeProduction(self, user_in):
        ...
        if idx >= 0:
            ...
            self.production_art = self.LoadTarget("mat", id)
    ...
    def StartSim(self, playsound):
        sim_label = 'VT_' + str(self.simCtr)
        self.simCtr = self.simCtr + 1
        if self.ssm is None:
            self.ssm = diva_ssm.SpeechSoundMap()
        ssm = self.ssm

        if self.cerebellum is None:
            self.cerebellum = ceb.Cerebellum()
        ...
        if playsound:
            vocal_tract.dosound(art_accum, False)
        self.reset()
    ...
    def ResetTarget(self):
        self.production_art[0, 0].Art.fill(0.0)
        self.PrepareSimulation()
        print("Target reset.")
